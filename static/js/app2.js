...
button.on("click", () => {
    ...
    var filteredData = alienData;

    if (dateValue != "") {
        var filteredData = filteredData.filter(filteredData => filteredData.datetime == dateValue);
        if (cityValue != ""){
            var filteredData = filteredData.filter(filteredData => filteredData.city == cityValue);
            if (stateValue != "") {
                var filteredData = filteredData.filter(filteredData => filteredData.state == stateValue);
                if (countryValue != "") {
                    var filteredData = filteredData.filter(filteredData => filteredData.country == countryValue);
                    if (shapeValue != "") {
                        var filteredData = filteredData.filter(filteredData => filteredData.shape == shapeValue);
                    }
                }
                else {
                    if (shapeValue != "") {
