...
button.on("click", () => {
    ...
    var inputValues = ["datetime","city","state","country","shape"];
    var filteredValues = [dateValue,cityValue,stateValue,countryValue,shapeValue];
    var filteredData = alienData;
    
    var zippedValues = filteredValues.map(function(value,input) {
        return[value,inputValues[input]];
    }); 

    console.log(zippedValues);

    zippedValues.forEach((value) => {
        var fValue = value[0];
        var iValue = value[1];
        console.log(fValue);
        console.log(iValue);
        if (fValue != "") {
            //var filteredData = filteredData.filter(filteredData => filteredData.iValue == fValue);
            var filteredData = filteredData.filter(data => data.iValue == fValue);
            console.log(filteredData)
        }
        else {
